...
describe('text handlebars helper', () => {
  ...
  it('should split text and render each item', async () => {
    expect(await parseTemplate('{{#each (text "john@briskine.com" "split" "@")}}{{.}}\n{{/each}}')).to.equal('john\nbriskine.com\n')
  })
  it('should split text and render first item', async () => {
    expect(await parseTemplate('{{lookup (text "john@briskine.com" "split" "@") 0}}')).to.equal('john')
  })
  it('should split text and render second item', async () => {
    expect(await parseTemplate('{{lookup (text "john@briskine.com" "split" "@") 1}}')).to.equal('briskine.com')
  })
  ...
  it('should replace character first occurrence in text', async () => {
    expect(await parseTemplate('{{text "john@brisk@ne.com" "replace" "@" "%"}}')).to.equal('john%brisk@ne.com')
  })

  // replaceAll
  it('should replace all character occurrences in text', async () => {
    expect(await parseTemplate('{{text "john@brisk@ne.com" "replaceAll" "@" "%"}}')).to.equal('john%brisk%ne.com')
  })

  // startsWith
  it('should print conditional startsWith subexpression', async () => {
    expect(await parseTemplate('{{#if (text "john@brisk@ne.com" "startsWith" "john")}}Hello John{{/if}}')).to.equal('Hello John')
  })

  // replicate the legacy domain helper with text
  it('should print prettified domain name', async () => {
    expect(await parseTemplate(`
